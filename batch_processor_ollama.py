#!/usr/bin/env python3
"""
OLLAMA-based Batch Processor for SMJ Papers (1985-1994)
Cost-effective processing using local OLLAMA models
"""
...
# Import our OLLAMA-based classes
from Kb_ollama import LiteratureAgentOllama, PaperMetadata, ResearchQuestion, Methodology, Finding, Contribution, ResearchEntity, Relationship
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('batch_processing_ollama.log'),
...
class BatchProcessorOllama:
    """Production-grade batch processor using OLLAMA for cost-effective processing"""
    
    def __init__(self, base_dir: Path, neo4j_uri: str = None, neo4j_user: str = None, 
                 neo4j_password: str = None, ollama_model: str = "llama3.1:8b"):
        ...
        self.agent = LiteratureAgentOllama(
    ...
    def find_papers_in_window(self, start_year: int, end_year: int) -> List[Path]:
        ...
        for pdf_file in self.base_dir.glob("*.pdf"):
            ...
            try:
                ...
                logger.warning(f"Could not parse year from filename: {pdf_file.name}")
                continue
        
        # Also look in subdirectories (in case papers are organized in folders)
    ...
    def process_single_paper(self, pdf_path: Path, window_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process the paper using OLLAMA
            result = self.agent.process_and_ingest(pdf_path)
            
            processing_time = time.time() - start_time
            
            # Update statistics
            self.stats['processed_papers'] += 1
            self.stats['total_questions'] += len(result.get('questions', []))
            self.stats['total_methodologies'] += len(result.get('methodologies', []))
            self.stats['total_findings'] += len(result.get('findings', []))
            self.stats['total_contributions'] += len(result.get('contributions', []))
            self.stats['total_entities'] += len(result.get('entities', []))
            self.stats['total_relationships'] += len(result.get('relationships', []))
            
            logger.info(f"✓ Successfully processed {paper_id} in {processing_time:.2f}s")
            ...
            return {
                ...
                'entities_count': {
                    ...
                    'methodologies': len(result.get('methodologies', [])),
    ...
    def process_temporal_window(self, start_year: int, end_year: int, max_workers: int = 2) -> Dict[str, Any]:
        ...
        logger.info(f"🚀 Processing temporal window: {window_name}")
        ...
        # Process papers with parallel execution (reduced workers for OLLAMA)
        results = []
    ...
    def process_all_windows(self, start_year: int, end_year: int, max_workers: int = 2) -> Dict[str, Any]:
        ...
        logger.info(f"🎯 Starting OLLAMA batch processing: {start_year}-{end_year}")
        ...
        for start_win, end_win in windows:
            try:
                ...
                # Add a small delay between windows to avoid overwhelming OLLAMA
                time.sleep(5)
        ...
        report = {
            ...
            'summary': {
                ...
                'entities_extracted': {
                    ...
                    'methodologies': self.stats['total_methodologies'],
                    'findings': self.stats['total_findings'],
                    'contributions': self.stats['total_contributions'],
    ...
    def _save_report(self, report: Dict[str, Any]):
        ...
        report_file = f"batch_processing_ollama_report_{timestamp}.json"
    ...
    def get_knowledge_graph_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self.agent.knowledge_graph.driver.session() as session:
...
def main():
    """Main function to run OLLAMA batch processing"""
    # Configuration
    BASE_DIR = Path("Strategic Management Journal")
    START_YEAR = 1985
    END_YEAR = 1994
    MAX_WORKERS = 2  # Reduced for OLLAMA stability
    OLLAMA_MODEL = "llama3.1:8b"  # or "mistral:7b", "codellama:7b"
    ...
    processor = BatchProcessorOllama(BASE_DIR, ollama_model=OLLAMA_MODEL)
    ...
    try:
        ...
        # Print summary
        print("\n" + "="*60)
        print("🎉 OLLAMA BATCH PROCESSING COMPLETED!")
        print("="*60)
        ...
        print("\n📈 Entities Extracted:")
        for entity_type, count in report['summary']['entities_extracted'].items():
